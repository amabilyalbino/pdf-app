...
export function AuthGate({ children }: AuthGateProps) {
  ...
  useEffect(() => {
    ...
    async function bootstrap() {
      try {
        ...
        const authType = url.searchParams.get("type") as EmailOtpType | null;
  ...
  if (desktopRuntime) {
    return <>{children({ authEmail: null, authProtected: false })}</>;
  }
      ...
      <div className="auth-shell">
        <div className="auth-card auth-card--setup">
          ...
          <p className="helper-copy">
            Also add your production domain and <code>http://localhost:3000/**</code> to the Supabase Auth redirect
            URLs.
          </p>
        </div>
      </div>
  ...
  if (session?.user?.email) {
    return <>{children({ authEmail: session.user.email, authProtected: true, onSignOut: handleSignOut })}</>;
  }
